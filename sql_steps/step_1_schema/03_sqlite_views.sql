-- ============================================================================
-- LedgerPro - Step 1: SQLite Database Views
-- Database Views for Reporting, Analytics & Accounting (SQLite Syntax)
-- ============================================================================
...
CREATE VIEW view_vendor_summary AS
SELECT 
    v.id AS vendor_id,
    ...
    v.phone,
    v.gstin,
    COALESCE(b.total_billed, 0) AS total_purchased,
    COALESCE(p.total_paid, 0) AS total_paid_to_vendor,
    (COALESCE(b.total_billed, 0) - COALESCE(p.total_paid, 0)) AS vendor_payable_balance
FROM vendors v
LEFT JOIN (
...
CREATE VIEW view_monthly_profit AS
SELECT 
    rev.month_year,
    COALESCE(rev.net_revenue, 0) AS total_sales,
    COALESCE(exp.total_purchases, 0) AS total_purchases,
    (COALESCE(rev.net_revenue, 0) - COALESCE(exp.total_purchases, 0)) AS estimated_gross_profit
FROM (
    SELECT strftime('%Y-%m', date) AS month_year, SUM(grand_total) AS net_revenue
    FROM invoices WHERE status != 'Cancelled'
    GROUP BY strftime('%Y-%m', date)
) rev
LEFT JOIN (
    SELECT strftime('%Y-%m', date) AS month_year, SUM(grand_total) AS total_purchases
...
SELECT 
    ...
    COALESCE(p.payment_number, 'PAY-' || p.id) AS doc_ref,
    p.date AS txn_date,
    0.0 AS debit_amount,
...
SELECT 
    ...
    COALESCE(p.payment_number, 'VPAY-' || p.id) AS doc_ref,
    p.date AS txn_date,
    p.amount AS credit_amount,
    0.0 AS debit_amount
FROM payments p
...
SELECT 
    'PAYMENT' AS module,
    p.id AS record_id,
    COALESCE(p.payment_number, 'PAY-' || p.id) AS ref_code,
    p.date AS txn_date,
    COALESCE(c.name, v.name, 'N/A') AS party_name,
